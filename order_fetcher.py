...
def extract_swap_eligible_orders(report: Report, filter: Optional[Filter]=None):
    """
...
def swap_filtering(responses: List[Response], orders_list: List[str]):
    ...
    for response, order in zip(responses, orders_list):
        ...
        elif data['iTotalDisplayRecords'] == 1:
            responses_from_swap["found"].append((order, data))
        else:
...
def main(reports_info:dict, downloader: ReportDownloader, swap_delivery: SwapOrder):
    ...
    for report_name in RUN_FOR:
        ...
        pbar = manager.counter(total=total_orders_count, desc='Fetching')
...
if __name__ == "__main__":
    ...
    reports_info = {
        ...
        'hotlink postpaid': {
            ...
            'filter': Filter('Fulfillment_Mode', 'exists', ('Standard Delivery', )),
        },
        'maxis postpaid': {
    ...
    filter_date_from = input("Enter starting datetime [DD/MM/YYYY HH:MM]: ")
    ...
    downloader = ReportDownloader(filter_date_from, filter_date_to)
